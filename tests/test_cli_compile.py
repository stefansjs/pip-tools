...
def test_command_line_pipfile_read(pip_conf, runner):
    with open("Pipfile", "w") as fh:
        pipfile_contents = """\
        [packages]
        small-fake-a = "==0.1"
        """
        fh.write(dedent(pipfile_contents))

    out = runner.invoke(cli, ["--no-emit-find-links"], catch_exceptions=False)
    ...
    assert out.stderr == dedent(expected_output), "{} != {}".format(
        out.stderr, dedent(expected_output)
    )
...
def test_command_line_pipfile_dev(pip_conf, runner):
    with open("Pipfile", "w") as fh:
        pipfile_contents = """\
        [dev-packages]
        small-fake-a = "==0.1"
    ...
    expected_output = dedent(
        ...
        # To update, run:
        #
        #    pip-compile --dev --no-emit-find-links
        #
        --index-url https://pypi.python.org/simple
    ...
    out = runner.invoke(cli, ["--no-emit-find-links", "--dev"], catch_exceptions=False)
    assert out.stderr == expected_output, f"{out.stderr} != {expected_output}"

    out = runner.invoke(
        cli, ["--no-emit-find-links", "--pipfile-dev"], catch_exceptions=False
    )
    assert out.stderr == expected_output
